...
│   ├── generate-llms.sh           # Creates llms.txt navigation map
...
Generates `llms.txt` - a navigation map for AI assistants to understand the documentation structure.
...
**What it does:**
- Creates a high-level overview of documentation structure
- Lists all API specifications with versions
- Includes links to guides, SDKs, and resources
- Provides contact information and allowed domains
...
4. **Generate LLMS files** - Creates `llms.txt` and `llms-full.txt` for AI assistants
...
- **`llms.txt`** - High-level navigation map for AI assistants (auto-generated by `scripts/generate-llms.sh`)
- **`llms-full.txt`** - Complete flattened documentation (auto-generated by `scripts/generate-llms-full.sh`)

These files enable AI assistants (GitHub Copilot, ChatGPT, Claude, etc.) to accurately answer questions about the NG SOS APIs.

**For maintainers:** Both `llms.txt` and `llms-full.txt` are regenerated automatically on each build. Manual updates are not necessary unless you want to customize the generation scripts.
